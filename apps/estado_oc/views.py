...
def generate_pdf(request):
    # Obtener todos los estados de orden de compra desde la base de datos
    estados_oc = EstadoOC.objects.all()

    # Crear un objeto HttpResponse con el tipo de contenido PDF
    response = HttpResponse(content_type='application/pdf')
    # Define el nombre del archivo PDF
    response['Content-Disposition'] = 'attachment; filename="reporte_estados_oc.pdf"'
    ...
    elements.append(Paragraph("<br/><br/><br/>Reporte de Estados de Orden de Compra", ParagraphStyle(name='TitleStyle', fontName='Helvetica-Bold', fontSize=16, textColor=colors.HexColor('#8C274C'), alignment=1)))

    # Agregar espacio entre el título y la tabla
    elements.append(Spacer(1, 36))

    # Agregar los detalles de cada estado de orden de compra al documento
    data = [["N° Estado OC", "Estado Orden de Compra", "Estado"]]
    for estado in estados_oc:
        data.append([
            estado.id_estado_oc,
            estado.nombre_estado_oc,
